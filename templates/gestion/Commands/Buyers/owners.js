...
exports.help = {
  name: 'owners',
  helpname: 'owners',
  description: 'Permet de voir la liste des owners',
  help: 'owners',
};
...
if (publicStatut) {
  ...
  if (args.length === 0) {
    db.all('SELECT id FROM owner', [], async (err, rows) => {
      if (err) {
        console.error('Erreur lors de la récupération de la liste owners :', err);
        return 
      }
      ...
      const generateEmbed = async (page) => {
        const embed = new Discord.EmbedBuilder()
          .setTitle('Owners')
          .setColor(config.color)
          .setFooter({ text: ` message.guild.name | ${rows.length} personnes - ${page}/${totalPages}`,
          iconURL: message.guild.iconURL({ dynamic: true })
});
